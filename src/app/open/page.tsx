...
export default function OpenTicket() {
  ...
  const handleSearchCustomer = async (data: FormData) => {
    ...
    if(response.data === null) {
      setError("email", { type: "custom", message: "Oops, cliente não foi encontrado!"})
      return;
    }
      ...
      <main className="flex flex-col mt-4 mb-2">
        {customer ? (
          <div className="bg-slate-200 py-6 px-4 rounded border-2 flex items-center justify-between">
            ...
            <button
              className="h-11 px-2 flex items-center justify-center btn-clear rounded hover:scale-125 duration-300 "
